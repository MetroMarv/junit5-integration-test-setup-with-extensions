package de.conciso.junit5extensionsforintegrationtests.parameterresolver;
...
class ShoppingCartControllerTest extends ITSetup {
  ...
  @Test
  void withAccessTokenWhenAddingItemToCartThenReturnHttpOk(UserPropertyResolver.KeycloakUser user) {
  ...
  @Test
  void multipleUserDoNotInterfereWithEachOther(UserPropertyResolver.KeycloakUser user) {
    HttpHeaders headers = new HttpHeaders();
    headers.setBearerAuth(user.token());
